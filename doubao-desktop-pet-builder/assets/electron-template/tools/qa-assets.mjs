  ...
  for (const frame of state.frames) {
    ...
    try {
      ...
      if (opaque > 0) {
        ...
        if (Math.abs(centerX - state.anchor.x) > 0.04) errors.push(`horizontal anchor drift: ${centerX.toFixed(3)}`);
        if (Math.abs(bottomY - state.anchor.y) > 0.02) errors.push(`bottom anchor drift: ${bottomY.toFixed(3)}`);
        const occupancy = Math.max(maxX - minX + 1, maxY - minY + 1) / 512;
        if (occupancy > targetOccupancy + 0.025) errors.push(`visible subject exceeds target occupancy: ${occupancy.toFixed(3)}`);
      }
      ...
      if (detachedGround) {
        ...
        regressionFixture ? warnings.push(message) : errors.push(message);
      }
      ...
      records.push({ state: state.id, frame, ok: errors.length === 0, errors, warnings, sha256: createHash('sha256').update(data).digest('hex'), transparentRatio: transparent / (info.width * info.height), bounds, componentCount: components.length, flatGround, coloredGround });
    } catch (error) {
      records.push({ state: state.id, frame, ok: false, errors: [error instanceof Error ? error.message : String(error)], warnings: [] });
    }
  }
}
...
for (const state of spec.states) {
  ...
  if (widths.length > 1 && (Math.max(...widths) / Math.min(...widths) > maximumScaleRatio || Math.max(...heights) / Math.min(...heights) > maximumScaleRatio)) {
    const message = `visible subject scale drifts too much within ${state.id} (limit ${((maximumScaleRatio - 1) * 100).toFixed(1)}%)`;
    for (const record of stateRecords) regressionFixture ? record.warnings.push(message) : record.errors.push(message);
  }
  ...
  for (let index = 0; index < stateRecords.length; index += 1) {
    ...
    if (previous !== undefined && !allowedBlinkClosure) {
      ...
      regressionFixture ? record.warnings.push(message) : record.errors.push(message);
    }
...
for (let index = 0; index < records.length; index += 1) {
  ...
  try {
    ...
    const labelText = `${record.state} · ${record.frame} · ${size}`.replaceAll('&','&amp;').replaceAll('<','&lt;');
    const label = Buffer.from(`<svg width="220" height="40" xmlns="http://www.w3.org/2000/svg"><rect width="220" height="40" fill="#fff"/><text x="110" y="25" text-anchor="middle" font-family="Arial,sans-serif" font-size="12" fill="#20242c">${labelText}</text></svg>`);
...
const report = { generatedAt: new Date().toISOString(), passed: records.every((item) => item.ok), regressionFixture, warningCount: records.reduce((total, item) => total + item.warnings.length, 0), assets: records };
